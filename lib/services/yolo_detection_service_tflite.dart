...
class YOLODetectionServiceTFLite {
  ...
  Future<void> initialize() async {
    try {
      ...
      print('🔍 YOLO initialized — output: $_outputShape, '
  ...
  Future<List<YOLODetectionResult>> detect(img.Image image) async {
    try {
      ...
      final detections = _postProcess(output[0], image.width, image.height);
  ...
  double _calculateIOU(YOLODetectionResult a, YOLODetectionResult b) {
    ...
    final x1 = (bbox_a[0] > bbox_b[0]) ? bbox_a[0] : bbox_b[0];
    final y1 = (bbox_a[1] > bbox_b[1]) ? bbox_a[1] : bbox_b[1];
    final x2 = (bbox_a[2] < bbox_b[2]) ? bbox_a[2] : bbox_b[2];
    final y2 = (bbox_a[3] < bbox_b[3]) ? bbox_a[3] : bbox_b[3];
    
    final interArea = (x2 - x1) * (y2 - y1);
    if (interArea <= 0) return 0.0;
    
    // Calculate union
    final areaA = (bbox_a[2] - bbox_a[0]) * (bbox_a[3] - bbox_a[1]);
    final areaB = (bbox_b[2] - bbox_b[0]) * (bbox_b[3] - bbox_b[1]);
  ...
  void dispose() {
    _interpreter.close();
    print('🧹 YOLO Detection Service disposed');
  }
}

  ...
  void _register() async {
    ...
    try {
      ...
      Navigator.of(context).pushAndRemoveUntil(
        _fadeRoute(const DoctorLoginScreen()), (_) => false);
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    child: _step == 0
                        ? AppCard(
                            ...
                            child: Column(children: [
                              AppTextField(label: 'FULL NAME', controller: _name, hint: 'Dr. Jane Smith', icon: Icons.person_outline_rounded, accent: _accent),
                              const SizedBox(height: 20),
                              AppTextField(label: 'MEDICAL LICENSE NO.', controller: _license, hint: 'e.g. ML-2024-001', icon: Icons.badge_outlined, accent: _accent),
                              const SizedBox(height: 20),
                              AppTextField(label: 'SPECIALIZATION', controller: _spec, hint: 'e.g. Cardiologist', icon: Icons.medical_services_outlined, accent: _accent),
                            ]),
                          )
                        : AppCard(
